...
def Menu():
    ...
    """to know about your screen resolution use the following import shutil
    col = shutil.get_terminal_size().col print(<string>.center(col))"""

    print("*" * 140)
    print("MAIN MENU".center(140))
    print("1. Insert Record/Records".center(140))
    print("2. Display Records as per Customer Name".center(140))
    print("3. Display all Records".center(140))
    print("4. Delete Record".center(140))
    print("5. Update Record".center(140))
    print("6. Search Record Details as per the account number".center(140))
    print("7. Search Record Details as per the Customer Name".center(140))
    print("8. Debit/Withdraw from the account".center(140))
    print("9. Credit into the account".center(140))
    print("10. Exit".center(140))
    print("*" * 140)


def SortAcc(F):
    # function to arrange records as per ascending order of Account Number
    try:
...
def SortBal(F):
    # function to arrange records as per ascending order of Customer Balance
    try:
        with open(F, "rb+") as fil:
            rec = pickle.load(fil)
...
def Insert(F):
    try:
        fil = open(
            F, "ab+"
        )  # will create file if not existing else read the records from the existing file.
        print(fil.tell())
        if fil.tell() > 0:
            fil.seek(0)
...
def Display(F):
    # Function to Display the records in the Binary File
    try:
        with open(F, "rb") as fil:
            print("=" * 140)
...
def Delete(F):
    # Function to delete the Record from the File, if it exists
    try:
        with open(F, "rb+") as fil:
            Rec = pickle.load(fil)
...
def SearchAcc(F):
    # Function to Search for the Record from the File with respect to the account number
    try:
        with open(F, "rb") as fil:
            Rec = pickle.load(fil)
...
def SearchName(F):
    # Function to Search for the Record from the File with respect to the customer name
    try:
        with open(F, "rb") as fil:
            Rec = pickle.load(fil)
...
def Debit(F):
    ...
    try:
        with open(F, "rb+") as fil:
            ...
            acc = int(
                input("Enter the account no from which the money is to be debited")
            )
            for i in range(0, len(Rec)):
                if Rec[i][0] == acc:
...
def Credit(F):
    # Function to Credit the amount into the account
    try:
        with open(F, "rb+") as fil:
            Rec = pickle.load(fil)
            acc = int(
                input("Enter the account no from which the money is to be credited")
            )
            for i in range(0, len(Rec)):
                if Rec[i][0] == acc:
